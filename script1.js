...
function createGrid() { for (let i = 1; i <= totalCells; i++) { const cell = document.createElement('div'); cell.classList.add('cell'); cell.textContent = i; gridOverlay.appendChild(cell); } 
} // Function to draw snakes and ladders 
function drawSnakesAndLadders() { for (const [start, end] of Object.entries(snakesAndLadders)) { const startPos = getCoordinates(parseInt(start)); const endPos = getCoordinates(parseInt(end)); const element = document.createElement('div'); element.classList.add(start < end ? 'ladder' : 'snake'); // Calculate the length and angle of the line 
const dx = endPos.x - startPos.x; const dy = endPos.y - startPos.y; const length = Math.sqrt(dx * dx + dy * dy); const angle = Math.atan2(dy, dx) * (180 / Math.PI); // Position and style the line 
element.style.width = `${length}px`; element.style.transform = `rotate(${angle}deg)`; element.style.left = `${startPos.x}px`; element.style.top = `${startPos.y}px`; element.style.transformOrigin = '0 0'; gridOverlay.appendChild(element); } } // Initialize game 
